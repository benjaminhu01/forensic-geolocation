...
if __name__ == '__main__':
    ...
    parser.add_argument('--seeds',
            help="Density of seeds per partition.")
    parser.add_argument('--region', type=float, nargs='*', 
            help="Probability of prediction region to calculate.")
    parser.add_argument('--taxa-threshold', type=float, default=0.05, 
            help='Remove taxa that are not present in at least this proportion of samples.')
    parser.add_argument('--domain-fp')
    parser.add_argument('--area', nargs='*')
    parser.add_argument('--centroids')
    parser.add_argument('--centroids-fp')
    parser.add_argument('--weight-by')
    parser.add_argument('--weight-by-fp')   
    parser.add_argument('--weight-by-col')
    parser.add_argument('--knn-n-neighbors', type=int, default=10)
    parser.add_argument('--rf-n-estimators', type=int, default=100)
    parser.add_argument('--nn-epochs', type=int, default=20)
    ...
    parser.add_argument('--dnn-batch-size', type=int, default=64)
    parser.add_argument('--dnn-verbose', action='store_true')
    parser.add_argument('--area-clf-epochs', type=int, default=20)
    parser.add_argument('--area-clf-batch-size', type=int, default=64)
    parser.add_argument('--area-clf-verbose', action='store_true')
    parser.add_argument('input_')
    parser.add_argument('output')
    args = parser.parse_args()

    args.area = [_[1:len(_)-1] for _ in args.area]
    print(args.region)
    print(type(args.region))
    print(args.area)
    print(type(args.area))

    cross_validate(args.input_,args.output,args.folds, args.partitions, args.seeds, args.region, args.domain_fp,
            args.area, args.centroids, args.centroids_fp, args.weight_by, args.weight_by_fp, args.weight_by_col,
            args.knn_n_neighbors, args.rf_n_estimators, args.nn_epochs, args.nn_batch_size, args.nn_verbose,
            args.dnn_epochs, args.dnn_batch_size, args.dnn_verbose, args.area_clf_epochs, 
            args.area_clf_batch_size, args.area_clf_verbose,args.taxa_threshold)
